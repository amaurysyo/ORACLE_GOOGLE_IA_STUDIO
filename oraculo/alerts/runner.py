...
class WsEventSource(EventSource):
    ...
    def __init__(
        ...
        queue_high_watermark_pct: float = 0.7,
        queue_backpressure_sleep_ms: int = 1,
        ...
        self._queue_high_watermark = max(1, int(self._queue_maxsize * queue_high_watermark_pct))
        self._queue_backpressure_sleep = max(0.0, queue_backpressure_sleep_ms / 1000.0)
        self._drop_log_interval_s = drop_log_interval_s
        self._queue: asyncio.Queue[Any] = asyncio.Queue(maxsize=self._queue_maxsize)
    ...
    async def next_batch(self, max_batch: int = 256, max_wait_ms: float = 10.0) -> list[Any]:
        """Devuelve un micro-batch de eventos para reducir overhead por evento."""
        ...
        deadline = time.perf_counter() + max(0.0, max_wait_ms / 1000.0)
    ...
    async def _put_event(self, ev: Any) -> None:
        # Evita flood de logs: si la cola está llena, descartar el más viejo y registrar métricas
        if self._queue.full():
            try:
                self._queue.get_nowait()
                self._dropped += 1
                obs_metrics.alerts_queue_dropped_total.labels(stream="all").inc()
            except asyncio.QueueEmpty:
                pass
            ...
            if now - self._last_drop_log >= self._drop_log_interval_s:
                logger.warning(
                    "[alerts-ws] queue full (size=%d), evicting oldest; drops=%d",
                    self._queue.qsize(),
                    self._dropped,
                )
        ...
        now = time.perf_counter()
        if qsize >= self._queue_high_watermark and self._queue_backpressure_sleep > 0:
            if now - self._last_bp_log >= self._drop_log_interval_s:
                logger.warning(
                    "[alerts-ws] backpressure active (size=%d/%d, drops=%d)",
                    qsize,
                    self._queue_maxsize,
                    self._dropped,
                )
                self._last_bp_log = now
            await asyncio.sleep(self._queue_backpressure_sleep)
...
async def run_pipeline(
    ...
    async def _run_ws_pipeline(
        ...
        symbol: str,
        queue_maxsize: int,
        queue_high_watermark_pct: float,
        queue_backpressure_sleep_ms: int,
        batch_size: int,
        batch_wait_ms: float,
    ) -> None:
        ws_source = WsEventSource(
            depth_levels=depth_levels,
            depth_ms=depth_ms,
            symbol=symbol,
            queue_maxsize=queue_maxsize,
            queue_high_watermark_pct=queue_high_watermark_pct,
            queue_backpressure_sleep_ms=queue_backpressure_sleep_ms,
        )
        await ws_source.start()
        logger.info(
            f"[alerts] WS hot path enabled (depth_levels={depth_levels}, depth_ms={depth_ms}, symbol={symbol}, "
            f"queue_maxsize={queue_maxsize}, batch_size={batch_size}, batch_wait_ms={batch_wait_ms})"
        )
        last_snapshot = time.perf_counter()
        last_opt_poll = time.perf_counter()
        try:
            while True:
                batch = await ws_source.next_batch(max_batch=batch_size, max_wait_ms=batch_wait_ms)
                if not batch:
                    continue
    ...
    queue_high_watermark_pct = 0.7
    queue_backpressure_sleep_ms = 1
    queue_batch_size = 256
    queue_batch_wait_ms = 10.0
    if cfg_mgr is not None and getattr(cfg_mgr, "cfg", None) is not None:
        try:
            ...
            queue_maxsize = int(alerts_cfg.get("queue_maxsize", queue_maxsize))
            queue_high_watermark_pct = float(alerts_cfg.get("queue_high_watermark_pct", queue_high_watermark_pct))
            queue_backpressure_sleep_ms = int(alerts_cfg.get("queue_backpressure_sleep_ms", queue_backpressure_sleep_ms))
            queue_batch_size = int(alerts_cfg.get("queue_batch_size", queue_batch_size))
            queue_batch_wait_ms = float(alerts_cfg.get("queue_batch_wait_ms", queue_batch_wait_ms))
        except Exception:
            pass
