...
UPCOMING_DOC_RULES = [
    ...
    {"rule": "R36", "name": "Gamma flip (GEX)", "event_type": "gamma_flip", "side": "na", "status": "implemented"},
...
def eval_rules(ev: Dict[str, Any], ctx: RuleContext) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # ---------- R34: Basis dislocation DOC ----------
        if et == "basis_dislocation":
            _append("R34", "na", ev, severity_=_sev_from_val(val, 0.60, 0.80))
            return out
        ...
        # ---------- R36: Gamma flip ----------
        if et == "gamma_flip":
            _append("R36", "na", ev, severity_=_sev_from_val(val, 0.60, 0.80))
            return out
