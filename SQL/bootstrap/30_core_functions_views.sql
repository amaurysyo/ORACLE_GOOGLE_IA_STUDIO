...
CREATE FUNCTION oraculo.upsert_rule_alert(p_instrument_id public.instrument_id_t, p_event_time timestamp with time zone, p_rule_code text, p_severity public.severity_t, p_dedup_key text, p_context jsonb, p_latency_ms integer, p_profile text) RETURNS bigint
    LANGUAGE plpgsql
    AS '
DECLARE v_id bigint;
BEGIN
  ...
  VALUES (p_instrument_id,p_event_time,p_rule_code,p_severity,p_dedup_key,p_event_time,p_event_time,1,COALESCE(p_context,''{}''::jsonb),p_latency_ms,p_profile)
