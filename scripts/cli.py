...
def ingest_run() -> None:
    ...
    setup_logging_json(
        ...
        logcfg.get("rotation", "10 MB"),
        logcfg.get("retention", "7 days"),
        logcfg.get("json", True),
        name="ingest",   # <-- LOG DEDICADO PARA INGESTA
    )
    ...
    async def _run() -> None:
        ...
        batcher.register_stmt(
           "deriv_ticker",
           """INSERT INTO deribit.options_ticker(instrument_id,event_time,mark_iv,delta,gamma,vega,theta,bid,ask,underlying_price,meta,open_interest)
...
def alerts_group() -> None:
    """Detectores + Motor de reglas + Enrutamiento Telegram."""


@alerts_group.command("run")
def alerts_run() -> None:
    _load_env()
    # En vez de cargar cfg suelto, usamos ConfigManager (con rules.yaml)
    from oraculo.config_hot import ConfigManager
    cfgmgr = ConfigManager(ROOT)

    cfg = cfgmgr.cfg

    # Mismo bloque de config que ingest, pero log separado (alerts.log)
    logcfg = (cfg.observability or {}).get("logging", {}) if cfg.observability else {}
    setup_logging_json(
        ROOT,
        logcfg.get("level", "INFO"),
        ...
        logcfg.get("retention", "7 days"),
        logcfg.get("json", True),
        name="alerts",   # <-- LOG DEDICADO PARA ALERTAS
    )

    async def _run() -> None:
        from oraculo.alerts.runner import run_pipeline
        db = DB(cfg.app.storage.dsn)
        await db.connect()
