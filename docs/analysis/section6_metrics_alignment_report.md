...
## Section 7 progress
- **Cobertura actual**: R1/R2 (break_wall+basis_vel), R3/R4 (absorción), R5–R8 (slicing agresivo/pasivo), R9/R10 (dominance), R11/R12 (spoofing), R13/R14 (depletion proxy), R15–R18 (basis extremo/mean-revert), R28/R29 (oi_spike, gated por `detectors.oi_spike.enabled=false`), R30/R31 (top_traders, gated por `detectors.top_traders.enabled=false`), R32/R33 (liq_cluster, gated por `detectors.liq_cluster.enabled=false`), R34 (basis_dislocation, gated por `detectors.basis_dislocation.enabled=false`), R35 (skew_shock, gated por `detectors.skew_shock.enabled=false`), R36 (gamma_flip, gated por `detectors.gamma_flip.enabled=false`) y R37 (term_structure_invert, gated por `detectors.term_structure_invert.enabled=false`) están implementadas en el engine con semántica legacy o parcial respecto al DOC.【F:oraculo/rules/engine.py†L16-L272】【F:config/rules.yaml†L130-L246】
- **Brechas DOC**: Sección 7 cerrada (Opción A) tras conectar R36 `gamma_flip`; los detectores macro siguen feature-flagged (`enabled=false`) hasta calibración, incluido gamma_flip.【F:oraculo/rules/engine.py†L16-L272】【F:config/rules.yaml†L130-L246】
...
- **Preparación Opción A**: slot R36 activado sin renumerar reglas existentes; resta calibrar thresholds de gamma_flip antes de habilitarlo por defecto.【F:oraculo/rules/engine.py†L16-L33】【F:config/rules.yaml†L174-L214】
...
- **R35 (skew_shock)**: detector macro sobre RR25d (delta y velocidad) con clamps de seguridad, auditoría completa (buckets, delta/velocidad, thresholds normalizados) y cooldown `retrigger_s`; mapea a R35 y se mantiene desactivado por defecto hasta calibración.【F:oraculo/detect/macro_detectors.py†L1034-L1179】【F:config/rules.yaml†L199-L215】【F:oraculo/alerts/cpu_worker.py†L618-L692】【F:oraculo/rules/engine.py†L257-L265】
- **R19 (iv_spike DOC)**: detector macro `iv_spike` sobre `options_iv_surface` (ventana `window_s`, `dv_warn/strong`, compuerta opcional de velocidad) con fallback ATM-ish a `options_ticker.mark_iv`, auditoría completa (`source`, `n_used`, filtros) y feature flag `detectors.iv_spike.enabled=false` hasta calibración.【F:oraculo/detect/macro_detectors.py†L139-L165】【F:oraculo/detect/macro_detectors.py†L1236-L1531】【F:config/rules.yaml†L246-L279】【F:oraculo/alerts/cpu_worker.py†L172-L208】【F:oraculo/alerts/cpu_worker.py†L614-L749】【F:oraculo/rules/engine.py†L195-L200】
- **R37 (term_structure_invert)**: detector macro sobre spread de IV short-long en `deribit.options_iv_surface`, con clamps de IV/spread/velocidad, ventana `lookback_s`, opción de gate por velocidad y cooldown `retrigger_s`; mapea a R37 con severidad por intensidad y está deshabilitado por defecto (`detectors.term_structure_invert.enabled=false`).【F:oraculo/detect/macro_detectors.py†L1184-L1364】【F:config/rules.yaml†L218-L242】【F:oraculo/alerts/cpu_worker.py†L535-L707】【F:oraculo/rules/engine.py†L262-L265】
- **Dependencia de surface**: `skew_shock` y `term_structure_invert` requieren `deribit.options_iv_surface`; el surface builder incremental (feature-flag `deribit_surface_builder.enabled=false`) evita escaneos completos y permite poblar la vista 1m sin cambiar semántica existente.【F:oraculo/deribit/surface_builder.py†L10-L241】【F:config/config.yaml†L77-L91】
