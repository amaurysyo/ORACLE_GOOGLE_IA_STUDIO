...
| R20 | Skew shock 25Δ | R35 | `skew_shock` (na) | IMPLEMENTED | Detector macro `skew_shock` sobre ΔRR_25d en ventana `window_s`, velocidad Δ/Δt, clamps y cooldown `retrigger_s`; emite lado bull/bear y mapea a R35 con severidad por intensidad. Gateado por `detectors.skew_shock.enabled=false`.|【F:oraculo/detect/macro_detectors.py†L1034-L1179】【F:oraculo/alerts/cpu_worker.py†L618-L692】【F:config/rules.yaml†L199-L215】【F:oraculo/rules/engine.py†L16-L33】【F:oraculo/rules/engine.py†L257-L265】 |
| R21 | Gamma flip (GEX) | R36 | `gamma_flip` (na) | IMPLEMENTED (GATED) | Detector macro `gamma_flip` agrega proxy net_gex=Σ gamma·oi·spot² con filtros lookback/expiry/moneyness/delta y clamps de seguridad; gatea por `min_instruments`/`min_abs_net_gex`, hysteresis y estabilidad de muestras antes de emitir flip estable, aplica cooldown y severidad por intensidad. Mapeado a R36 y feature-flag `detectors.gamma_flip.enabled=false` por defecto.|【F:oraculo/detect/macro_detectors.py†L137-L303】【F:oraculo/alerts/cpu_worker.py†L380-L720】【F:config/rules.yaml†L174-L214】【F:oraculo/rules/engine.py†L16-L78】【F:oraculo/rules/engine.py†L242-L272】 |
| R22 | Term structure invertida | R37 | `term_structure_invert` (na) | IMPLEMENTED (GATED) | Detector macro `term_structure_invert` consulta `deribit.options_iv_surface` por buckets short/long, aplica clamp y spread/velocidad en ventana, y mapea a R37 con severidad por intensidad; gateado por `detectors.term_structure_invert.enabled=false`.|【F:oraculo/detect/macro_detectors.py†L1184-L1364】【F:oraculo/alerts/cpu_worker.py†L535-L707】【F:config/rules.yaml†L218-L242】【F:oraculo/rules/engine.py†L16-L33】【F:oraculo/rules/engine.py†L262-L265】 |
...
| R20 Skew shock 25Δ | R35 | `skew_shock` (na) | OI/IV de opciones (skew 25Δ) en ingesta Deribit | ΔRR25d y velocidad en ventana, clamps | P2 | **IMPLEMENTED (GATED)** — detector `skew_shock` con cooldown, clamps y auditoría completa; gated por `detectors.skew_shock.enabled=false` hasta calibración. |
| R21 Gamma flip (GEX) | R36 | `gamma_flip` (na) | Greeks (gamma) ya presentes en tabla de options | Calcular GEX agregado, detectar cambio de signo con spot~ATM | P2 | **IMPLEMENTED** — detector `gamma_flip` (feature-flag `detectors.gamma_flip.enabled=false`) calcula proxy net_gex con filtros lookback/expiry/moneyness/delta y clamps, detecta flip estable con hysteresis+cooldown y mapea a R36 con severidad por intensidad.【F:oraculo/detect/macro_detectors.py†L137-L303】【F:config/rules.yaml†L174-L214】【F:oraculo/rules/engine.py†L16-L78】【F:oraculo/rules/engine.py†L242-L272】 |
| R22 Term structure invertida | R37 | `term_structure_invert` (na) | IV surface (front/back) si se expone en ingesta | Derivar term structure y tendencia de vol | P2 | **IMPLEMENTED (GATED)** — detector `term_structure_invert` usa buckets front/back Deribit, spread/velocidad y cooldown; mapeo a R37 con severidad por intensidad y gate `detectors.term_structure_invert.enabled=false`. |
