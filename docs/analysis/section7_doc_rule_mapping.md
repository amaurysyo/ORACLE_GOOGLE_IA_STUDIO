...
| R21 | Gamma flip (GEX) | N/A | — | NOT_IMPLEMENTED | No se ingesta ni calcula GEX.|【F:oraculo/rules/engine.py†L243-L272】 |
...
- Las reglas R28/R29 están implementadas y gobernadas por `detectors.oi_spike.enabled` (default false) para no alterar producción hasta habilitación explícita; R30/R31 `top_traders`, R34 `basis_dislocation`, R35 `skew_shock` y R37 `term_structure_invert` siguen el mismo patrón de feature flag (`detectors.top_traders.enabled=false`, `detectors.basis_dislocation.enabled=false`, `detectors.skew_shock.enabled=false`, `detectors.term_structure_invert.enabled=false`). R36 permanece reservado como stub sin lógica en el engine.
