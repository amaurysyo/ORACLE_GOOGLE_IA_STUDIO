...
- Las reglas R28/R29 están implementadas y gobernadas por `detectors.oi_spike.enabled` (default false) para no alterar producción hasta habilitación explícita; R30/R31 `top_traders`, R34 `basis_dislocation` y R35 `skew_shock` siguen el mismo patrón de feature flag (`detectors.top_traders.enabled=false`, `detectors.basis_dislocation.enabled=false`, `detectors.skew_shock.enabled=false`). R36–R37 permanecen reservadas como stubs sin lógica en el engine.
